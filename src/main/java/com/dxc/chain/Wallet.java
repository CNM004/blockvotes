...
public class Wallet {
	...
	public int getBalance() {
		int total = 0;
		for (Map.Entry<String, TransactionOutput> item : HomeController.UTXOs.entrySet()) {
	...
	// returns transaction histories
		public float getTransactionHistories() {
			float total = 0;
			for (Map.Entry<String, TransactionOutput> item : HomeController.UTXOs.entrySet()) {
				TransactionOutput UTXO = item.getValue();
				if (UTXO.isMine(publicKey)) { // if output belongs to me ( if coins
					...
					UTXOs.put(UTXO.id, UTXO); // add it to our list of unspent
												// transactions.
					total += UTXO.value;
					System.out.println(UTXO.reciepient+ ": "+UTXO.value);
				}
			}
			return total;
		}
